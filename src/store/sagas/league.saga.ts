...
import {GET_ALL_TEAMS_IN_LEAGUE, GET_LEAGUE_SEASONS, GET_LEAGUE_TABLE} from '../modules/league.module';
import {
  GetAllTeamsInLeagueAction, GetAllTeamsInLeagueResponse,
  ...
  LeagueSeason,
  LeagueTable,
  ObjectizedLeagueTable, ObjectizedTeamInLeague, TeamInLeague
...
export function* getAllTeamsInLeague(action: GetAllTeamsInLeagueAction) {
  ...
  try {
    const response: GetAllTeamsInLeagueResponse = yield call(() => leagueService.getAllTeamsInLeague(action.payload));
